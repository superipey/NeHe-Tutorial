...
int LoadGLTextures()                                    // Load Bitmaps And Convert To Textures
{
	...
	texture[1] = SOIL_load_OGL_texture("Data/Image.bmp", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y);
	...
	int			nCmdShow)						// Window Show State
{
	...
	if (!CreateGLWindow(L"NeHe's Line Tutorial", 640, 480, 16, fullscreen))
	{
	...
	while (!done)												// Loop That Runs While done=FALSE
	{
		...
		else													// If There Are No Messages
		{
			...
			if (!gameover && active)							// If Game Isn't Over And Programs Active Move Objects
			{
				for (loop1 = 0; loop1<(stage*level); loop1++)		// Loop Through The Different Stages
				{
					...
					if ((enemy[loop1].fx == player.fx) && (enemy[loop1].fy == player.fy))
					{
						...
						PlaySound(L"Data/Die.wav", NULL, SND_SYNC);	// Play The Death Sound
					}
				}
			...
			{
				PlaySound(L"Data/Complete.wav", NULL, SND_SYNC);	// If So, Play The Level Complete Sound
			...
			if ((player.fx == hourglass.x * 60) && (player.fy == hourglass.y * 40) && (hourglass.fx == 1))
			{
				...
				PlaySound(L"Data/freeze.wav", NULL, SND_ASYNC | SND_LOOP);
			...
			{													// Variable Greater Than 6000 Divided By The Current Level?
				PlaySound(L"Data/hourglass.wav", NULL, SND_ASYNC);	// If So, Play The Hourglass Appears Sound
