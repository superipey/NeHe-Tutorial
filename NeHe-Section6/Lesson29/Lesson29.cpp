// Lesson29.cpp : Defines the entry point for the application.
//
...
P_TEXTURE_IMAGE AllocateTextureBuffer(GLint w, GLint h, GLint f)
{
	...
	if (ti != NULL) {
		...
		if (c != NULL) {
			...
			MessageBox(NULL, L"Could Not Allocate Memory For A Texture Buffer", L"BUFFER ERROR", MB_OK | MB_ICONINFORMATION);
			return NULL;
		}
	}
	else
	{
		MessageBox(NULL, L"Could Not Allocate An Image Structure", L"IMAGE STRUCTURE ERROR", MB_OK | MB_ICONINFORMATION);
		return NULL;
	}
...
int ReadTextureData(char *filename, P_TEXTURE_IMAGE buffer)
{
	...
	{
		MessageBox(NULL, L"Unable To Open Image File", L"IMAGE ERROR", MB_OK | MB_ICONINFORMATION);
	}
...
int InitGL(GLvoid)												// This Will Be Called Right After The GL Window Is Created
{
	...
	{															// Nothing Read?
		MessageBox(NULL, L"Could Not Read 'Monitor.raw' Image Data", L"TEXTURE ERROR", MB_OK | MB_ICONINFORMATION);
		return FALSE;
	}
	...
	{															// Nothing Read?
		MessageBox(NULL, L"Could Not Read 'GL.raw' Image Data", L"TEXTURE ERROR", MB_OK | MB_ICONINFORMATION);
		return FALSE;
	}
	...
	int			nCmdShow)				// Window Show State
{
	...
	if (!CreateGLWindow(L"Andreas Löffler, Rob Fletcher & NeHe's Blitter & Raw Image Loading Tutorial", 640, 480, 32, fullscreen))
	{
	...
	while (!done)										// Loop That Runs While done=FALSE
	{
		...
		if (keys[VK_F1])								// Is F1 Being Pressed?
		{
			...
			if (!CreateGLWindow(L"Andreas Löffler, Rob Fletcher & NeHe's Blitter & Raw Image Loading Tutorial", 640, 480, 16, fullscreen))
			{
				return 0;								// Quit If Window Was Not Created
			}
		}
