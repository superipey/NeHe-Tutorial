...
BOOL CreateGLWindow(LPCWCHAR title, int width, int height, int bits, bool fullscreenflag)
{
					...
					int			nCmdShow)			// Window Show State
{
	...
	if (!CreateGLWindow(L"Magic Room",640,480,16,fullscreen))
	{
  ...
  if (!hook_toball1)
  {
	...
	while (RestTime>ZERO)
	{
	   ...
	   if (FindBallCol(Pos2,BallTime,RestTime,BallColNr1,BallColNr2))
			  {
				  if (sounds)
					  PlaySound(L"Data/Explode.wav",NULL,SND_FILENAME|SND_ASYNC);
