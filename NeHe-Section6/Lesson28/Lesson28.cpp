...
BOOL LoadGLTexture(GLuint *texPntr, char* name)                                    // Load Bitmaps And Convert To Textures
{
	/* load an image file directly as a new OpenGL texture */
	GLuint _texPntr = SOIL_load_OGL_texture
	(
		name,
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
	...
	if (_texPntr == 0) return false;

	// Typical Texture Generation Using Data From The Bitmap
	glBindTexture(GL_TEXTURE_2D, _texPntr);
